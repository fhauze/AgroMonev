...
export default function Dashboard() {
  ...
  const landsSafe = safeArray(lands);
  const plantsSafe = safeArray(plants);

  const totalArea = landsSafe.reduce(
  (sum, land) => sum + (land.area_hectares || 0),
    0
  );

  const verifiedFarmers = farmersSafe.filter(
    f => f.verification_status === "verified"
  ).length;

  const validLands = landsSafe.filter(
    l => l.validation_status === "valid"
  ).length;

  const alivePlants = plantsSafe.filter(
    p => p.status === "alive"
  ).length;

  // const totalArea = lands.reduce((sum, land) => sum + (land.area_hectares || 0), 0);
  // const verifiedFarmers = farmers.filter(f => f.verification_status === "verified").length;
  // const validLands = lands.filter(l => l.validation_status === "valid").length;
  ...
  const commodityData = plantsSafe
  .reduce((acc, plant) => {
    const commodity = plant.commodity_name || "Lainnya";
    const existing = acc.find(c => c.name === commodity);
  ...
  // Issues
  const pendingValidation = landsSafe.filter(l => l.validation_status === "pending" || l.validation_status === "need_review").length;
  const unverifiedFarmers = farmersSafe.filter(f => f.verification_status === "pending").length;
  const sickPlants = plantsSafe.filter(p => p.status === "sick").length;
          ...
          <Card className="border-0 shadow-sm">
            ...
            <CardContent>
              <div className="space-y-3">
                {farmersSafe.slice(0, 5).map((farmer, idx) => (
